...
- Stockage : Upstash Redis (offre gratuite, région Frankfurt), partagé entre le scrap (écriture) et l'API Vercel (lecture). Clés `schedule-index` (liste des promotions, avec `hasCourses`) et `schedule:<promotion>` (planning), voir `shared/redis-keys.mjs`. Le hash des jetons y vivra avec le flux ICS.
...
- Entre promotions, deux cours n'en font qu'un si le code ET la matière sont identiques, ou si la même matière a lieu au même moment (au moins une occurrence identique : semaine, jour, heures). Dédoublonné une fois pour toutes dans `server/lessons.mjs`, à réutiliser par le flux ICS.
...
- Abonnement (jeton + flux ICS + ajout au calendrier) : retiré le 2026-09-23 pour être repris de zéro ; le dernier écran affiche une URL factice (`MOCK_FEED_URL`). La première version reste dans l'historique de `main` (commits c7e97b5 à 2c09dd5).
- Reste avant la bêta : abonnement, sélection modifiable après coup (revenir sur un flux existant à partir de son URL pour changer la sélection — pas encore possible), page de suppression des données, motif de reconnaissance pour le cursus « Accessoires de mode » (aucun libellé identifié pour l'instant).
